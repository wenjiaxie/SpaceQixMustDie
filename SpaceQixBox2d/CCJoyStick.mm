...
-(void)updateTouchPoint:(CGPoint)touchPoint
{
	...
	if (ccpDistance(touchPoint, ccp(self.position.x,self.position.y)) > (MoveAreaRadius-BallRadius)) {
		currentPoint =ccpAdd(ccp(0,0),ccpMult(ccpNormalize(ccpSub(ccp(touchPoint.x-self.position.x,touchPoint.y-self.position.y), ccp(0,0))), MoveAreaRadius-BallRadius));
	} else {
		currentPoint = ccp(touchPoint.x-self.position.x,touchPoint.y-self.position.y);
	}
    ...
    if( (angle < 45.0 && angle > 0.0) || (angle > -45.0 && angle < 0.0))
        angle = 0.0;
    else if( (angle > 45.0 && angle < 90.0) || (angle > 90.0 && angle < 135.0) )
        angle = 90.0;
    else if ((angle > 135.0 && angle < 180.0) || (angle > -180.0 && angle < - 135.0))
        angle = 180.0;
    else if((angle < -45.0 && angle > -90.0) || (angle > -135.0 && angle < -90.0) )
        angle = -90.0;
...
{  
	...
	if (ActiveRadius>0)
	{
		if (ccpDistance(touchPoint, ccp(self.position.x,self.position.y)) < ActiveRadius) {
			return YES;
		}
	}
